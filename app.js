...
app.get("/movies/",async (request, response) => {
    ...
    FROM
        movie_table`
    const movie = database.all(movieDetails)
...
app.post("/movies/",async (request,response) => {
    ...
    const updateMovies = `
        INSERT INTO 
            movie_table(director_id,movie_name,lead_actor)
        VALUES
            ${directorId},${movieName},${leadActor}`
    const movie = await database.run(updateMovies)
...
app.put("/movies/:movieId/",async (request,response) => {
    ...
    SET
        ...
        lead_actor = ${leadActor},
         `
    const movie = await database.run(upMovies)
...
app.delete("/movies/:movieId/", async (request,response))=> {
    ...
    WHERE 
        movie_id = ${movieId}`
    const movie = await database.run(deleteMovies)
...
app.get("/directors/:directorId/movies/",async (request,response) => {
    ...
    const movie =  await database.get(directorMovie)
    response.send(movie.map(each) => {
        convertDbObjectToResponseObject(each)
    })
})
